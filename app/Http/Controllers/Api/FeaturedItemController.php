...
class FeaturedItemController extends Controller {
    ...
    public function store(Request $request) {
        $data = $request->validate([ 'title' => 'required|string', 'description' => 'required|string', 'image' => 'nullable|image' ]);
        $dbData = ['title' => $data['title'], 'description' => $data['description']];
        if ($request->hasFile('image')) {
            $dbData['image_url'] = $request->file('image')->store('homepage/featured', 'public');
        }
        $item = FeaturedItem::create($dbData);
