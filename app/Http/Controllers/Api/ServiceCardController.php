...
class ServiceCardController extends Controller
{
    ...
    {
        return response()->json(ServiceCard::all());
    }
    ...
    public function store(Request $request)
    {
        ...
        // We remove the temporary 'image' key before saving to the database.
        unset($data['image']);
    ...
    public function update(Request $request, ServiceCard $serviceCard)
    {
        ...
        if ($request->hasFile('image')) {
            if ($serviceCard->image_url) {
                Storage::disk('public')->delete($serviceCard->image_url);
            }
            $data['image_url'] = $request->file('image')->store('services/cards', 'public');
        }
